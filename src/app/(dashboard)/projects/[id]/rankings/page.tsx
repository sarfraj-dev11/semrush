...
import { KeywordDialog } from "../keywords/keyword-dialog";
import { DailyTrackingBanner } from "./daily-tracking-banner";
...
export default async function RankingsPage({
  ...
  try {
    ...
    console.error("⚠️ [Rankings] Failed to auto-sync from Firebase:", err);
  }
          ...
          <form className="flex items-center gap-2" method="get">
            ...
            {q ? (
              <Button variant="ghost" size="sm" asChild>
                <Link href={`/projects/${projectId}/rankings`}>Reset</Link>
              </Button>
            ) : null}
          </form>
              ...
              <Table>
                ...
                <Tbody>
                  {filteredRows.map((r, ri) => (
                    <Tr key={`${r.keyword.id}-${r.device}-${r.country}-${ri}`}>
                      <Td className="font-medium">
                        <Link
                          href={`/projects/${projectId}/rankings?q=${encodeURIComponent(
                            r.keyword.keyword,
                      ...
                      <Td className="text-center">
                        {r.latestPosition != null ? (
                          (() => {
                            const pageNum = Math.ceil(r.latestPosition / 10);
                            return (
                              <Badge
                                tone={
                                  pageNum === 1
                          ...
                          <Badge tone="neutral" className="text-[11px] font-normal text-muted-foreground">
                            &gt; Page 10
                          </Badge>
                        )}
                      </Td>
